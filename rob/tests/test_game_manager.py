...
class GameManagerTest(TestCase):
    ...
    def test_expired_rounds_returns_only_games_where_round_is_over(self):
        expired_round_game = Game.objects.create(started_at = two_hours_ago, ends_at=hour_ago)
        games = Game.objects.get_expired_games()
        self.assertIn(expired_round_game, games)

    def test_expired_rounds_does_not_return_finished_game(self):
        finished_game = Game.objects.create(started_at = two_hours_ago, ends_at=hour_ago, in_progress=False)
        games = Game.objects.get_expired_games()
        self.assertNotIn(finished_game, games)

    def test_expired_rounds_does_not_return_game_where_round_isnt_over(self):
        live_round_game = Game.objects.create(started_at = now, ends_at=hour_from_now)
        games = Game.objects.get_expired_games()
        self.assertNotIn(live_round_game, games)

    def test_first_round_games_returns_only_games_in_round_one(self):
        game_first_round_open_to_join = Game.objects.create(round_no=1, started_at = now, ends_at=hour_from_now)
        game_second_round = Game.objects.create(round_no=2, started_at = now, ends_at=hour_from_now)
